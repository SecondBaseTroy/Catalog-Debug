...
APPLICATION_NAME = "Shoe Catalog Application"
app = Flask(__name__)
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'username' in login_session:
            ...
            # ----- for debugging on the arguments and name ---------
            # flash(*args)
            # flash("username is " + username)
        else:
...
def getArbitaryItem():
    ...
    category_output["items.title"] = items.title
    category_output["items.owner"] = items.owner
    category_output["items.description"] = items.description
    output_json.append(category_output)
    return jsonify(Categories=output_json)
...
def getMainPage():
    ...
    try:
        ...
        # print the user in flash for debugging
        # flash("user - " + user)
    except KeyError:
        user = None
    if request.method == 'GET':
        ...
        # check if an error is in the access token info,, then abort.
        ...
        if stored_credentials is not None and gplus_id == stored_gplus_id:
            response = make_response(
                                     json.dumps('User is already connected.'),
                                     200)
            response.headers['Content-Type'] = 'application/json'
            return response
...
def getItemDetails(item_title):
    ...
    return render_template(
        'item_detail.html', item=item, owner=owner, category=category
    )
...
def editItem(item_title):
    ...
    category = \
        session.query(Category).filter_by(id=editedItem.category_id).one()
    categories = session.query(Category).all()
    if request.method == 'POST':
...
def deleteItem(item_title):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'delete_item.html', item_title=item_title, owner=owner, user=user)
...
if __name__ == '__main__':
    app.secret_key = 'ncAkvL1QSygeFQcjOVFCrC5M'
    app.debug = True
